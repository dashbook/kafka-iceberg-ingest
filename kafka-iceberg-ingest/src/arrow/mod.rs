...
use arrow::datatypes::{
    ...
    Field, Float32Type, Float64Type, Int16Type, Int32Type, Int64Type, Int8Type,
...
use self::schema::to_arrow_schema;

type RecordSlice<'a> = &'a [&'a Vec<(String, Value)>];

pub fn schema_lookup(schema: &AvroSchema) -> Result<&BTreeMap<String, usize>, anyhow::Error> {
    match schema {
        AvroSchema::Record {
            lookup: ref schema_lookup,
            ..
        } => Ok(schema_lookup),
        _ => Err(anyhow!("expected avro schema to be a record")),
    }
}

pub fn avro_to_arrow_batch(
    schema: &AvroSchema,
    records: RecordSlice,
) -> Result<RecordBatch, anyhow::Error> {
    let lookup = schema_lookup(schema)?;
    let schema = Arc::new(to_arrow_schema(schema)?);
    let columns = build_struct_array(lookup, records, schema.fields(), &vec![])?;
...
fn build_boolean_array(
    ...
    for row in rows {
        if let Some(value) = field_lookup(lookup, col_name, row) {
    ...
    T::Native: num_traits::cast::NumCast,
{
    Arc::new(
        rows.iter()
            .map(|row| {
                field_lookup(lookup, col_name, row).and_then(|value| resolve_item::<T>(value))
            })
...
#[inline(always)]
fn build_string_dictionary_builder<T>(row_len: usize) -> StringDictionaryBuilder<T>
where
...
fn build_wrapped_list_array(
    ...
    match *key_type {
        DataType::Int8 => {
            ...
            list_array_string_array_builder::<Int8Type>(lookup, &dtype, col_name, rows)
        }
        DataType::Int16 => {
            let dtype = DataType::Dictionary(Box::new(DataType::Int16), Box::new(DataType::Utf8));
            list_array_string_array_builder::<Int16Type>(lookup, &dtype, col_name, rows)
        }
        DataType::Int32 => {
            let dtype = DataType::Dictionary(Box::new(DataType::Int32), Box::new(DataType::Utf8));
            list_array_string_array_builder::<Int32Type>(lookup, &dtype, col_name, rows)
        }
        DataType::Int64 => {
            let dtype = DataType::Dictionary(Box::new(DataType::Int64), Box::new(DataType::Utf8));
            list_array_string_array_builder::<Int64Type>(lookup, &dtype, col_name, rows)
        }
        DataType::UInt8 => {
            let dtype = DataType::Dictionary(Box::new(DataType::UInt8), Box::new(DataType::Utf8));
            list_array_string_array_builder::<UInt8Type>(lookup, &dtype, col_name, rows)
        }
        DataType::UInt16 => {
            let dtype = DataType::Dictionary(Box::new(DataType::UInt16), Box::new(DataType::Utf8));
            list_array_string_array_builder::<UInt16Type>(lookup, &dtype, col_name, rows)
        }
        DataType::UInt32 => {
            let dtype = DataType::Dictionary(Box::new(DataType::UInt32), Box::new(DataType::Utf8));
            list_array_string_array_builder::<UInt32Type>(lookup, &dtype, col_name, rows)
        }
        DataType::UInt64 => {
            let dtype = DataType::Dictionary(Box::new(DataType::UInt64), Box::new(DataType::Utf8));
            list_array_string_array_builder::<UInt64Type>(lookup, &dtype, col_name, rows)
        }
    ...
    D: ArrowPrimitiveType + ArrowDictionaryKeyType,
{
    let mut builder: Box<dyn ArrayBuilder> = match data_type {
        ...
        DataType::Dictionary(_, _) => {
            let values_builder = build_string_dictionary_builder::<D>(rows.len() * 5);
            Box::new(ListBuilder::new(values_builder))
        }
        e => {
...
where
    ...
    T: ArrowPrimitiveType + ArrowDictionaryKeyType,
{
    let mut builder: StringDictionaryBuilder<T> = build_string_dictionary_builder(rows.len());
    for row in rows {
        if let Some(value) = field_lookup(lookup, col_name, row) {
...
fn build_string_dictionary_array(
    ...
    if let DataType::Utf8 = *value_type {
        match *key_type {
            DataType::Int8 => build_dictionary_array::<Int8Type>(lookup, rows, col_name),
            DataType::Int16 => build_dictionary_array::<Int16Type>(lookup, rows, col_name),
            DataType::Int32 => build_dictionary_array::<Int32Type>(lookup, rows, col_name),
            DataType::Int64 => build_dictionary_array::<Int64Type>(lookup, rows, col_name),
            DataType::UInt8 => build_dictionary_array::<UInt8Type>(lookup, rows, col_name),
            DataType::UInt16 => build_dictionary_array::<UInt16Type>(lookup, rows, col_name),
            DataType::UInt32 => build_dictionary_array::<UInt32Type>(lookup, rows, col_name),
            DataType::UInt64 => build_dictionary_array::<UInt64Type>(lookup, rows, col_name),
...
fn build_nested_list_array<OffsetSize: OffsetSizeTrait>(
    ...
    let array_data = match list_field.data_type() {
        ...
        DataType::LargeUtf8 => flatten_string_values(rows)
            ...
            .into_data(),
        DataType::List(field) => {
            let child = build_nested_list_array::<i32>(lookup, &flatten_values(rows), field)?;
            child.to_data()
        }
        DataType::LargeList(field) => {
            let child = build_nested_list_array::<i64>(lookup, &flatten_values(rows), field)?;
            child.to_data()
        }
        DataType::Struct(fields) => {
            ...
            let rows: Vec<Vec<(String, Value)>> = rows
                ...
                .collect();
            let rows = rows.iter().collect::<Vec<&Vec<(String, Value)>>>();
            let arrays = build_struct_array(lookup, &rows, fields, &[])?;
    ...
    let arrays: ArrowResult<Vec<ArrayRef>> = struct_fields
        ...
        .map(|field| {
            let arr = match field.data_type() {
                ...
                DataType::Boolean => build_boolean_array(lookup, rows, field.name()),
                DataType::Float64 => {
                    build_primitive_array::<Float64Type>(lookup, rows, field.name())
                }
                DataType::Float32 => {
                    build_primitive_array::<Float32Type>(lookup, rows, field.name())
                }
                DataType::Int64 => build_primitive_array::<Int64Type>(lookup, rows, field.name()),
                DataType::Int32 => build_primitive_array::<Int32Type>(lookup, rows, field.name()),
                DataType::Int16 => build_primitive_array::<Int16Type>(lookup, rows, field.name()),
                DataType::Int8 => build_primitive_array::<Int8Type>(lookup, rows, field.name()),
                DataType::UInt64 => build_primitive_array::<UInt64Type>(lookup, rows, field.name()),
                DataType::UInt32 => build_primitive_array::<UInt32Type>(lookup, rows, field.name()),
                DataType::UInt16 => build_primitive_array::<UInt16Type>(lookup, rows, field.name()),
                DataType::UInt8 => build_primitive_array::<UInt8Type>(lookup, rows, field.name()),
                // TODO: this is incomplete
                DataType::Timestamp(unit, _) => match unit {
                    TimeUnit::Second => {
                        build_primitive_array::<TimestampSecondType>(lookup, rows, field.name())
                    }
                    TimeUnit::Microsecond => build_primitive_array::<TimestampMicrosecondType>(
                        lookup,
                        rows,
                        field.name(),
                    ),
                    TimeUnit::Millisecond => build_primitive_array::<TimestampMillisecondType>(
                        lookup,
                        rows,
                        field.name(),
                    ),
                    TimeUnit::Nanosecond => {
                        build_primitive_array::<TimestampNanosecondType>(lookup, rows, field.name())
                    }
                },
                DataType::Date64 => build_primitive_array::<Date64Type>(lookup, rows, field.name()),
                DataType::Date32 => build_primitive_array::<Date32Type>(lookup, rows, field.name()),
                DataType::Time64(unit) => match unit {
                    TimeUnit::Microsecond => {
                        build_primitive_array::<Time64MicrosecondType>(lookup, rows, field.name())
                    }
                    TimeUnit::Nanosecond => {
                        build_primitive_array::<Time64NanosecondType>(lookup, rows, field.name())
                    }
                    t => {
                        return Err(ArrowError::SchemaError(format!(
                ...
                DataType::Time32(unit) => match unit {
                    TimeUnit::Second => {
                        build_primitive_array::<Time32SecondType>(lookup, rows, field.name())
                    }
                    TimeUnit::Millisecond => {
                        build_primitive_array::<Time32MillisecondType>(lookup, rows, field.name())
                    }
                    t => {
                        return Err(ArrowError::SchemaError(format!(
                ...
                DataType::Utf8 | DataType::LargeUtf8 => Arc::new(
                    rows.iter()
                        .map(|row| {
                            let maybe_value = field_lookup(lookup, field.name(), row);
                            match maybe_value {
                                None => Ok(None),
                ...
                DataType::List(ref list_field) => {
                    match list_field.data_type() {
                        DataType::Dictionary(ref key_ty, _) => {
                            build_wrapped_list_array(lookup, rows, field.name(), key_ty)?
                        }
                        _ => {
                            ...
                            let extracted_rows = rows
                                .iter()
                                .map(|row| {
                                    field_lookup(lookup, field.name(), row).unwrap_or(&Value::Null)
                                })
                                .collect::<Vec<&Value>>();
                            build_nested_list_array::<i32>(lookup, &extracted_rows, list_field)?
                        }
                    }
                }
                DataType::Dictionary(ref key_ty, ref val_ty) => {
                    build_string_dictionary_array(lookup, rows, field.name(), key_ty, val_ty)?
                }
                DataType::Struct(fields) => {
                    let len = rows.len();
                    ...
                    let struct_rows = rows
                        .iter()
                        .enumerate()
                        .map(|(i, row)| (i, field_lookup(lookup, field.name(), row)))
                        .map(|(i, v)| {
                    ...
                    let arrays = build_struct_array(lookup, &struct_rows, fields, &[])?;
...
fn read_primitive_list_values<T>(rows: &[&Value]) -> ArrayData
where
...
fn field_lookup<'b>(
    ...
    lookup.get(name).and_then(|i| row.get(*i)).map(|o| &o.1)
}
